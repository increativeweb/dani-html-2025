 ...
 * WooW Form shortcode
 */
if ( !function_exists( 'wow_form_cb' ) ) {
    function wow_form_cb( $atts, $content = null ) {
        $rev = '';

        $rev .= '<form id="wow_form" class="wow-form">';
            $rev .= '<input name="wow" type="input" placeholder="'. esc_html__( 'Hey! Do you guys also cover....', 'wp-rock' ) .'" />';
            $rev .= '<button type="submit">?</button>';
        $rev .= '</form>';

        return $rev;
    }

    add_shortcode( 'wow_form', 'wow_form_cb' );
}