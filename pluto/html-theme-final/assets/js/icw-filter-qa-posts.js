(function ($) {
	...
	$doc.ready(function () {
		 ...
		 * Retrieve posts
		 */
		function get_faq_posts($params) {
			$container = $('#container-async');
			$content = $container.find('.content');
			$status = $container.find('.status');
			$pager = $container.find('.infscr-pager a');

			$status.text('Loading question posts ...');

			/**
			 * Reset Pager for infinite scroll
			 */
			if ($params.page === 1 && $pager.length) {
				$pager.removeAttr('disabled').html('More questions');
			}
			if ($pager.length) {
				$method = 'infscr';
			}
			...
			$.ajax({
				...
				complete: function (data, textStatus) {
					...
					if (msg === 'No posts found') {
						$content.html('<div class="post-card-col text-center pt-50 pb-50 w-100"><div class="alert alert-danger">No question posts found.</div></div>');
					}
		...
		$(document).on('click', '#container-async a[data-filter], #container-async .pagination a', function (event) {
			...
			if ($(this).data('infscr') === 'load') {
				$(this).html('<span class="arrow-icon icw-loading"></span> Load questions');
			}
