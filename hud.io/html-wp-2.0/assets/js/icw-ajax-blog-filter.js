(function ($) {
	...
	jQuery(document).ready(function ($) {
		...
		AJAX FETCH
		================================ */
		function get_posts(params, append = false) {
				...
				success: function (res) {
					...
					if (res.next && res.next > 0) {
						...
						if ($pager.data('infscr') == 'scroll') {
							...
							$pager.show().removeAttr('disabled').addClass('btn btn-white mx-auto').text('Load articles');
						}
					}
					else if (res.next == 0) {
						isEnd = true;
