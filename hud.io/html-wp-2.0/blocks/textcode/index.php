<?php
/**
 * HTML Code Block – Simplified & Dynamic
 */
if ( ! empty( $block['data']['icw_preview'] ) ) {
	echo '<img src="' . get_template_directory_uri() . '/blocks/textcode/' . $block['data']['icw_preview'] . '" alt=""/>';
	return;
}

$group_title_info      	= get_field( 'title_group' );
$group_classic_code 	= get_field( 'classic_code' );
$group_html_code 		= get_field( 'html_code' );
...
?>
<section <?php echo $gs_anchor; ?> <?php echo $data_theme; ?> class="theme-section html-code-section <?php echo $gs_class; ?>" <?php echo $gs_style; ?>> 
	<div class="container">
		<?php
		...
		?>
		<?php if(!empty($group_classic_code)) { ?>
			<div class="classic-code">
				<?php echo $group_classic_code; ?>
			</div>
		<?php } ?>
		<?php if(!empty($group_html_code)) { ?>
			<div class="html-code">
				<?php echo $group_html_code; ?>
			</div>
		<?php } ?>
	</div>
